import { POSTS_CREATE, POSTS_POST } from '@/server/backend/apiRoutes';
...
const PromptInput: React.FC<PromptInputProps> = ({
  ...
  const createPostRequest = async ({ prompt }: { prompt: string }) => {
    try {
  ...
  const onSubmitHandler = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await createPostRequest({ prompt: prompt });
    console.log(res);
  ...
  const postImageToFeedHandler = async () => {
    ...
    console.log(res);
    closeModal();
    // Link this to the feed page - feed refreshed here
  };
  return (
